...
class LingBotVideoInt8Linear(nn.Module):
    """Row-wise INT8 weight-only linear used by dense 1.3B checkpoints.

    Weights remain INT8 while resident/offloaded and are dequantized to the
    activation dtype only at the matmul boundary.  This is a compatibility and
    memory-saving format; it does not claim an INT8 activation kernel speedup.
    """
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if not input.is_floating_point():
            input = input.to(dtype=self.compute_dtype)
        weight = self.weight.to(dtype=input.dtype)
        weight = weight * self.weight_scale.to(dtype=input.dtype).unsqueeze(-1)
        bias = self.bias
...
def _fp8_scaled_linear(
    input: torch.Tensor,
    weight: torch.Tensor,
    weight_scale: torch.Tensor,
    bias: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Native Blackwell W8A8 matmul with dynamic tensor-wise activation scaling."""
    if input.device.type != "cuda":
        # CPU fallback is useful for diagnostics; production inference uses CUDA.
        restored = weight.to(dtype=torch.bfloat16) * weight_scale.to(torch.bfloat16)
        return F.linear(input.to(torch.bfloat16), restored, bias)
    original_shape = input.shape
    input_2d = input.reshape(-1, original_shape[-1]).to(torch.bfloat16)
    input_scale = (input_2d.float().abs().amax() / FP8_E4M3_MAX).clamp_min(1e-12).reshape(1)
    input_fp8 = (
        (input_2d.float() / input_scale)
...
class LingBotVideoFP8Linear(nn.Module):
    """Tensor-wise E4M3 W8A8 Linear backed by native ``torch._scaled_mm``."""
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return _fp8_scaled_linear(input, self.weight, self.weight_scale, self.bias)


def _replace_dense_block_linears_with_int8(module: nn.Module) -> None:
    """Replace Linear children recursively while preserving checkpoint names."""
    for name, child in tuple(module.named_children()):
        if isinstance(child, nn.Linear):
            replacement = LingBotVideoInt8Linear(
                child.in_features,
                child.out_features,
            ...
            _replace_dense_block_linears_with_int8(child)


def _replace_dense_speed_linears_with_fp8(blocks: nn.ModuleList) -> None:
    """Quantize only projections that benefit from fused FP8 execution."""
    for block in blocks:
        for owner, names in (
            (block.attn, ("to_q", "to_k", "to_v")),
            (block.ffn, ("gate_proj", "up_proj")),
        ):
            for name in names:
...
def _linear_compute_dtype(module: nn.Module) -> torch.dtype:
    """Return matmul dtype independently from a quantized weight's storage dtype."""
    if isinstance(module, (LingBotVideoInt8Linear, LingBotVideoFP8Linear)):
        return module.compute_dtype
    return module.weight.dtype


def _resolve_attention_backend(
    requested_backend: Optional[str], attention_mask: Optional[torch.Tensor]
) -> Optional[str]:
    """Choose a Diffusers backend without silently dropping a padding mask.

    Regular Sage attention does not accept masks.  The varlen implementation
    does, so padded single-sample prompts and padded CFG batches are routed to
    it while unpadded calls retain the explicitly requested Sage backend.
    """

    backend = requested_backend.lower().strip() if requested_backend else None
    if backend == "sage" and attention_mask is not None:
        return "sage_varlen"
    return backend


def _expand_per_sample_conditioning(
    per_sample: torch.Tensor,
    sequence_length: int,
    packed_sequence_lengths: Optional[list[int]] = None,
    padding_size: int = 0,
) -> torch.Tensor:
    """Broadcast sample-level conditioning after its expensive projection.

    The common padded path keeps a singleton token dimension and relies on
    broadcasting. Packed/context-parallel paths require the historical token
    layout, which is reconstructed with cheap views/copies after projection.
    """

    if packed_sequence_lengths is None:
        return per_sample.unsqueeze(1)
    if len(packed_sequence_lengths) != per_sample.shape[0]:
        raise ValueError("Packed sequence lengths must match the conditioning batch size.")
    expanded = torch.cat(
        [
            per_sample[i : i + 1].unsqueeze(1).expand(1, length, -1)
            for i, length in enumerate(packed_sequence_lengths)
        ],
        dim=1,
    )
    if padding_size:
    ...
    if expanded.shape[1] != sequence_length:
        raise ValueError(
            f"Expanded conditioning has {expanded.shape[1]} tokens; expected {sequence_length}."
        )
    return expanded
...
class LingBotVideoAttention(nn.Module):
    ...
    def forward(
        ...
        B, S, _ = x.shape
        if isinstance(self.to_q, LingBotVideoFP8Linear):
            if not all(isinstance(item, LingBotVideoFP8Linear) for item in (self.to_k, self.to_v)):
                raise RuntimeError("FP8 attention requires q, k, and v projections together")
            weight = torch.cat((self.to_q.weight, self.to_k.weight, self.to_v.weight), dim=0)
            qkv = _fp8_scaled_linear(x, weight, self.to_q.weight_scale)
            q, k, v = qkv.view(B, S, 3, self.num_heads, self.head_dim).unbind(2)
        elif (
            os.environ.get("LINGBOT_FUSED_QKV_LINEAR") == "1"
            and not isinstance(self.to_q, LingBotVideoInt8Linear)
        ):
            weight = torch.cat(
        ...
        if packed_indices is None:
            resolved_backend = _resolve_attention_backend(attention_backend, attention_mask)
...
class LingBotVideoMLP(nn.Module):
    ...
    def forward(self, x):
        if isinstance(self.gate_proj, LingBotVideoFP8Linear):
            if not isinstance(self.up_proj, LingBotVideoFP8Linear):
                raise RuntimeError("FP8 MLP requires gate and up projections together")
            weight = torch.cat((self.gate_proj.weight, self.up_proj.weight), dim=0)
            projected = _fp8_scaled_linear(x, weight, self.gate_proj.weight_scale)
...
class LingBotVideoGroupedExperts(nn.Module):
    ...
    def __init__(self, num_experts, hidden_size, intermediate_size, quant=None):
        super().__init__()
        self.num_experts = num_experts
        self.quant = quant
        if quant == "int8_rowwise":
            # LOCAL PATCH: int8 expert weights + per-expert per-row fp32 scales.
            # dequant(w) = int8.to(bf16) * scale.unsqueeze(-1); scale reduces the
            # last (input) dim, so w1/w3 scales are [E, I] and w2 is [E, H].
            self.w1 = nn.Parameter(
                torch.empty(num_experts, intermediate_size, hidden_size, dtype=torch.int8),
                requires_grad=False,
            )
            ...
            self.w1_scale = nn.Parameter(
                torch.empty(num_experts, intermediate_size, dtype=torch.float32),
                requires_grad=False,
            )
            ...
            raise ValueError(f"unsupported experts_quant: {quant!r}")
        else:
    ...
    def dequant(self, name: str, dtype: torch.dtype = torch.bfloat16) -> torch.Tensor:
        """Full [E, rows, cols] weight in compute dtype (dequantized if int8)."""
        weight = getattr(self, name)
        if weight.dtype == torch.int8:
            scale = getattr(self, f"{name}_scale")
            return weight.to(dtype) * scale.to(dtype).unsqueeze(-1)
        return weight.to(dtype)

    def dequant_expert(self, name: str, expert_idx: int, dtype: torch.dtype) -> torch.Tensor:
        """Single expert's [rows, cols] weight in compute dtype."""
        weight = getattr(self, name)[expert_idx]
        if weight.dtype == torch.int8:
            scale = getattr(self, f"{name}_scale")[expert_idx]
            return weight.to(dtype) * scale.to(dtype).unsqueeze(-1)
        return weight.to(dtype)
...
class LingBotVideoSparseMoeBlock(nn.Module):
    def __init__(self, hidden_size, intermediate_size, num_experts, top_k,
                 ...
                 routed_scaling_factor, n_shared_experts, experts_quant=None):
        super().__init__()
        ...
        self.experts = LingBotVideoGroupedExperts(
            num_experts, hidden_size, moe_intermediate_size, quant=experts_quant
        )
    ...
    def _run_grouped_experts(self, tokens: torch.Tensor, counts: torch.Tensor) -> torch.Tensor:
        ...
        # LOCAL PATCH: dequant() replaces .bfloat16() so int8_rowwise expert
        # weights (int8 * rowwise scale) work on this path; identical for bf16.
        h = F.silu(
            torch._grouped_mm(
                padded_tokens.bfloat16(),
                self.experts.dequant("w1").transpose(-2, -1),
                offs=offsets,
            )
        )
        h = h * torch._grouped_mm(
            padded_tokens.bfloat16(),
            self.experts.dequant("w3").transpose(-2, -1),
            offs=offsets,
        )
        out = torch._grouped_mm(
            h,
            self.experts.dequant("w2").transpose(-2, -1),
            offs=offsets,
    ...
    def _run_experts_for_loop(self, tokens: torch.Tensor, counts: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx, expert_tokens in enumerate(splits):
            if expert_tokens.numel() == 0:
                continue
            # LOCAL PATCH: dequant_expert handles int8_rowwise weights and is a
            # plain dtype cast for bf16 checkpoints.
            dt = expert_tokens.dtype
            h = F.silu(expert_tokens @ self.experts.dequant_expert("w1", expert_idx, dt).transpose(-2, -1))
            h = h * (expert_tokens @ self.experts.dequant_expert("w3", expert_idx, dt).transpose(-2, -1))
            h = h @ self.experts.dequant_expert("w2", expert_idx, dt).transpose(-2, -1)
            outputs.append(h)
        if not outputs:
    ...
    def _get_sglang_w13(self) -> torch.Tensor:
        if getattr(self.experts, "quant", None) is not None:
            raise RuntimeError("sglang expert paths do not support quantized experts; use the diffusers backend.")
        key = (
...
class LingBotVideoBlock(nn.Module):
    def __init__(
        ...
        if layer_idx not in mlp_only_layers and (
            ...
            self.ffn = LingBotVideoSparseMoeBlock(
                ...
                n_group, topk_group, routed_scaling_factor,
                n_shared_experts, experts_quant=experts_quant,
            )
        else:
    ...
    def forward(
        ...
        if temb6.ndim != 3 or temb6.shape[0] not in (1, x.shape[0]) or temb6.shape[1] not in (1, x.shape[1]):
            raise ValueError(
                "LingBotVideoBlock expects broadcastable temb6 with shape "
                f"(B|1, S|1, 6D); got {tuple(temb6.shape)} for hidden states {tuple(x.shape)}."
            )
        ...
        mod = temb6 + self.scale_shift_table.unsqueeze(0)
        ...
        bulk_dtype = _linear_compute_dtype(self.attn.to_q)
...
class LingBotVideoTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        experts_quant: Optional[str] = None,
        dense_quant: Optional[str] = None,
        dense_fp8: Optional[str] = None,
    ):
        super().__init__()
        ...
        if dense_quant is not None:
            if dense_quant != "int8_rowwise":
                raise ValueError(f"Unsupported dense_quant format: {dense_quant}")
            if num_experts:
                raise ValueError("dense_quant is only supported for the dense LingBot model")
            _replace_dense_block_linears_with_int8(self.blocks)
        if dense_fp8 is not None:
            if dense_fp8 != "e4m3fn_tensorwise_fused":
                raise ValueError(f"Unsupported dense_fp8 format: {dense_fp8}")
            if num_experts or dense_quant is not None:
                raise ValueError("dense_fp8 requires an otherwise unquantized dense LingBot model")
            _replace_dense_speed_linears_with_fp8(self.blocks)
    ...
    def forward(
        ...
        padded_batch = bool(padded_batch and B > 1)
        packed_batch = B > 1 and not padded_batch
        ...
        if packed_batch:
            ...
            padded_rotary_parts = []
            for rotary_part, text_len in zip(rotary_parts, text_lens_list):
                padding = L - text_len
                if padding:
                    # Padded text queries are masked as K/V. Identity complex
                    # rotations keep their otherwise-unused Q positions inert.
                    rotary_part = torch.cat(
                        [
                            rotary_part,
                            torch.ones(
                                padding,
                                rotary_part.shape[-1],
                                dtype=rotary_part.dtype,
                                device=rotary_part.device,
                            ),
                        ],
                        dim=0,
                    )
                padded_rotary_parts.append(rotary_part)
            rotary = torch.stack(padded_rotary_parts, dim=0)
        else:
        ...
        if has_padding:
            ...
            # Diffusers native SDPA and Sage varlen both accept (B,S) boolean
            # masks. Keeping this canonical form lets the attention layer route
            # regular Sage to sage_varlen without rebuilding a large 4D mask.
        ...
        # These pointwise projections depend only on the sample timestep. Run
        # each once per sample, then broadcast the result across tokens.
        temb6_per_sample = self.time_modulation(t_emb)                         # (B, 6D)
        final_mod_per_sample = self.norm_out_modulation(t_emb)                 # (B, 2D)
        sample_seq_lens = [n_video + text_len for text_len in text_lens_list]
        if packed_batch:
            temb6 = _expand_per_sample_conditioning(
                temb6_per_sample,
                joint_seq_len,
                sample_seq_lens,
                padding_size=padding_size,
            )
            final_mod = _expand_per_sample_conditioning(
                final_mod_per_sample,
                joint_seq_len,
                sample_seq_lens,
                padding_size=padding_size,
            )
        elif packed_cp:
            # B=1 context parallelism historically keeps padded text tokens in
            # the local sequence. Preserve that token layout while avoiding
            # token-wise Linear projections.
            temb6 = temb6_per_sample.unsqueeze(1).expand(B, joint_seq_len, -1)
            final_mod = final_mod_per_sample.unsqueeze(1).expand(B, joint_seq_len, -1)
        else:
            temb6 = temb6_per_sample.unsqueeze(1)
            final_mod = final_mod_per_sample.unsqueeze(1)
        ...
        if packed_cp:
            final_mod = self.cp_final_mod(final_mod)
