...
class LingBotVideoPipeline(DiffusionPipeline):
    ...
    @classmethod
    def _apply_endpoint_conditioning(
        cls,
        ...
        if cond_latent is not None:
            latents = cls._apply_inpainting(latents, cond_latent)
        if cond_latent_last is not None:
            if cond_latent_last.shape[2] != 1:
                raise ValueError(
                    "`cond_latent_last` must contain exactly one temporal latent frame"
                )
            if latents.shape[2] < 2:
                raise ValueError("First/last-frame conditioning requires at least two latent frames")
            latents[:, :, -1:] = cond_latent_last.float()
        return latents
    ...
    def _build_prompt_inputs(
        ...
        if images is not None:
            ...
            visual_template = self.img_prompt_template * max(1, image_count)
    ...
    def __call__(
        ...
        batch_cfg_fraction: float = 1.0,
        batch_cfg_tail: bool = False,
        attention_backend: Optional[str] = None,
        batch_cfg_backend: str = "native",
        ...
        attention_backend = attention_backend.lower().strip() if attention_backend else None
        batch_cfg_backend = str(batch_cfg_backend).lower().strip()
        if batch_cfg_backend not in {"native", "sage_varlen"}:
            raise ValueError(
                "`batch_cfg_backend` must be either 'native' or 'sage_varlen', "
                f"got {batch_cfg_backend!r}."
            )
        requested_batch_cfg = bool(batch_cfg)
        batch_cfg_fraction = max(0.0, min(1.0, float(batch_cfg_fraction)))
        effective_batch_cfg = requested_batch_cfg
        batch_cfg_fallback_reason = None
        if requested_batch_cfg and do_cfg:
            latent_video_tokens = (
                ((int(num_frames) + 3) // 4)
                * (int(height) // 16)
                * (int(width) // 16)
            )
            if latent_video_tokens > 16_384:
                effective_batch_cfg = False
                batch_cfg_fallback_reason = (
                    f"{latent_video_tokens:,} video tokens exceed the safe 16,384-token "
                    "batched CFG limit"
                )
            elif batch_cfg_backend == "sage_varlen":
                try:
                    from sageattention import sageattn_varlen  # noqa: F401
                except Exception as exc:
                    effective_batch_cfg = False
                    batch_cfg_fallback_reason = f"sageattn_varlen unavailable: {exc}"
        self._last_batch_cfg_requested = requested_batch_cfg
        self._last_attention_backend = attention_backend
        self._last_batch_cfg_backend = batch_cfg_backend
        ...
        self._last_batch_cfg_steps = 0
        self._last_total_denoise_steps = 0
        ...
        if cond_latent_last is not None:
            cond_latent_last = cond_latent_last.to(device=device, dtype=torch.float32)
        latents = self._apply_endpoint_conditioning(
            latents,
            cond_latent,
            cond_latent_last,
        )
        ...
        total_denoise_steps = len(self.scheduler.timesteps)
        batch_cfg_steps = (
            min(total_denoise_steps, max(1, int(total_denoise_steps * batch_cfg_fraction)))
            if effective_batch_cfg and do_cfg and batch_cfg_fraction > 0.0
            else 0
        )
        self._last_batch_cfg_steps = batch_cfg_steps
        self._last_total_denoise_steps = total_denoise_steps
        for i, timestep in enumerate(self.progress_bar(self.scheduler.timesteps)):
            if batch_cfg_tail:
                use_batch_cfg_step = bool(
                    effective_batch_cfg and do_cfg and i >= total_denoise_steps - batch_cfg_steps
                )
            else:
                use_batch_cfg_step = bool(effective_batch_cfg and do_cfg and i < batch_cfg_steps)
            if cfg_parallel:
            ...
            if cfg_parallel:
                    ...
                    try:
                        with _transformer_autocast(device, transformer_dtype):
                            noise_batched = self.transformer(
                                ...
                                padded_batch=True,
                                attention_backend=batch_cfg_backend,
                                return_dict=False,
                            )[0].float()
                        ...
                        del cfg_latents, cfg_timesteps, cfg_embeds
                        torch.cuda.empty_cache()
                        effective_batch_cfg = False
                        batch_cfg_steps = i
                        batch_cfg_fallback_reason = "CUDA OOM; retried with sequential CFG"
                        self._last_effective_batch_cfg = False
                        self._last_batch_cfg_steps = batch_cfg_steps
                        self._last_batch_cfg_fallback_reason = batch_cfg_fallback_reason
                        with _transformer_autocast(device, transformer_dtype):
                ...
                if do_cfg and not use_batch_cfg_step:
                    negative_model_input = negative_embeds.to(transformer_dtype)
                    with _transformer_autocast(device, transformer_dtype):
                        noise_pred_uncond = self.transformer(
                            latent_model_input,
